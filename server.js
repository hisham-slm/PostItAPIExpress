...
app.get('/posts', authenticateToken, updateAccessToken, async (req, res) => {
    try {
        const posts = await Post.find().sort({ _id: -1 })
        let postDetails = []

        for (const post of posts) {
            const postedBy = await User.findOne({ _id: post.user }); 

            const likedUsersDetails = Array.isArray(post.likedUsers) ? 
                await Promise.all(post.likedUsers.map(async (userId) => {
                    const likedUser = await User.findById(userId, { username: 1 });
                    return likedUser.username;
                })) : [];
